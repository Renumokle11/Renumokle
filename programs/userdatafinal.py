...
class UserDetails:
    ...
    def Insert_data(self):
        ...
        try:
            ...
            create_new_file=input("Do you want to create a new user_data.csv file?(Yes/No):")
            if create_new_file=='Yes':
                with open(self.filename, 'w', newline='') as new_file:
                   header_writer = csv.DictWriter(new_file,fieldnames=['Name','Last_name', 'Pan_no','Mobile_number', 'Aadhar_number',
                                                   'House_no', 'House_name', 'Colony', 'Landmark', 'City', 'State'])

                header_writer.writeheader()
                print(f"New file{self.filename} created successfully.")

                for user in users:
                    header_writer.writerow({user['Name']},
                                    {user['Last_name']},
                                    {user['Pan_no']},
                                    {user['Mobile_number']},
                                    {user['Aadhar_number']},
                                    {user['address']['House_no']},
                                    {user['address']['House_name']},
                                    {user['address']['Colony']},
    ...
    def save_data(self, output_file1, output_file2):
        ...
        self.read_existing_data (self.filename)
        ...
        with open(output_file1, 'w', newline='') as file1, open(output_file2, 'w', newline='') as file2:
            ...
            for user in self.users:
                ...
                with open(self.filename,'w',newline='') as user_file:
                    user_writer=csv.DictWriter(user_file,fieldnames=['Name', 'Last_name', 'Pan_no', 'Mobile_number', 'Aadhar_number',
                                                                     'House_no', 'House_name', 'Colony', 'Landmark', 'City', 'State'])
                    user_writer.writeheader()
                    for user in self.users:
                        user_writer.writerow({'Name': user['Name'],
                    'Last_name': user['Last_name'],
                    'Pan_no': user['Pan_no'],
                    'Mobile_number': user['Mobile_number'],
                    'Aadhar_number': user['Aadhar_number'],
    ...
    def read_existing_data(self, filename):
        try:
            ...
            return None
    def merge_files(self,output_file1,output_file2):
        merged_data=""
        try:
            with open(output_file1,'r')as file1,open(output_file2,'r')as file2:
                merged_data+=file1.read()
                merged_data+=file1.read()
        except FileNotFoundError:
            pass
    ...
    def run_program(self):
        while True:
            ...
            print("3. Merge and Display Data")
            print("4. Exit")

            self.existing_data1 = self.read_existing_data("output_file1.csv")
            self.existing_data2 = self.read_existing_data("output_file2.csv")
            choice = self.get_valid_input("Enter your choice (1/2/3/4): ", self.validate_choice)
            if choice == '1':
                self.Insert_user()
            elif choice == '2':
                self.Fetch_user_data()
            elif choice == '3':
                merged_data=self.merge_files("output_file1.csv","output_file2.csv")
                if merged_data:
                    print("Merged data from output_file1.csv and output_file2.csv")
                    print(merged_data)
                else:
                    print("No data found in the merged files.")
                    self.save_data("output_file1.csv", "output_file2.csv")
